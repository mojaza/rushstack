...
class PhasedRequestBatchCoordinator {
  ...
  readonly #routingState: Pick<IGraphRoutingState, 'eventSequenceBySessionId'>;
  readonly #workspaceSession: IWorkspaceSession;
  ...
  public constructor(
    ...
    routingState: Pick<IGraphRoutingState, 'eventSequenceBySessionId'>,
    ...
    try {
      ...
      for (const entry of participants) {
        ...
        entry.requestSink = new PhasedRequestEventSink({
          activeOperationIds,
          client: entry.client,
          getNextSequence: () => getNextEventSequence(this.#routingState, entry.client.sessionId),
...
function getGraphRoutingState(
  ...
  if (!state) {
    ...
    const eventSequenceBySessionId: Map<string, number> = new Map();
    const graphExecutionScheduler: RequestScheduler = new RequestScheduler();
    const partialState: Pick<IGraphRoutingState, 'eventSequenceBySessionId'> = {
      eventSequenceBySessionId
    };
    state = {
...
function getNextEventSequence(
  state: Pick<IGraphRoutingState, 'eventSequenceBySessionId'>,
  sessionId: string
): number {
...
function collectOperationOutcomes(
  ...
  for (const operation of [...activeOperations].sort(compareOperations)) {
    ...
    const errorMessage: string | undefined = retained?.error?.message ?? observed?.errorMessage;
    outcomes.push({
